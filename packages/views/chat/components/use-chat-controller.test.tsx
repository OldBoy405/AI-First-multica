...
// Descending updated_at 鈫?sortChatSessions renders them sA, sB, sC.
...
describe("useChatController.advanceSelectionAfterArchive", () => {
  ...
  it("advances to the next chat and syncs the selected agent across agents", () => {
    ...
    // The next chat belongs to a different agent 鈥?selectedAgentId must follow
...
// its URL鈫抯tore effect clears it back to null. The auto-mark-read must NOT fire
// for that transiently-active session 鈥?otherwise the badge vanishes though the
...
describe("useChatController auto mark-read", () => {
  ...
  it("marks read a session that stays active past the tick", () => {
    ...
    // Deferred, not synchronous 鈥?nothing fires on the mount frame.
  ...
  it("does NOT mark read a session that was only momentarily active on mount", () => {
    ...
    // ...then the page's URL鈫抯tore effect clears it before the tick elapses.
    h.store.activeSessionId = null;
    rerender();
...
// MUL-4360 mount-race defer above 鈥?the read is scheduled a tick after the
// effect runs 鈥?so each assertion advances fake timers to let it (or not) fire.
describe("useChatController auto mark-read 鈥?foreground gating (MUL-4485)", () => {
...
describe("useChatController durable draft restores (#5219)", () => {
  ...
  // seen 鈥?the restore arrives purely through the draft-restores query on
  ...
  // so the row comes back on the next fetch 鈥?but it was already applied, and
  ...
  // restore the user is waiting on in a foreground surface 鈥?the same silent-theft
...
// session they sent from 鈥?otherwise the shared editor is showing a different
...
describe("useChatController.handleSend 鈥?compose target tracking", () => {
  beforeEach(() => {
    h.store.setActiveSession.mockClear();
    h.createSessionMutate.mockClear();
    h.createSessionMutate.mockResolvedValue({ id: "new-session" });
    ...
    return {
      ...
      send: () =>
        act(async () => {
          ...
          // Runs between ensureSession and the commit 鈥?the window the user
  ...
  it("scrubs the composer even if the agent picker moved mid-send", async () => {
    ...
    // The message goes to the agent selected when Send was pressed 鈥?a later
    // flick of the picker does not re-route work already on its way.
    expect(h.createSessionMutate).toHaveBeenCalledWith(
      expect.objectContaining({ agent_id: "agent-a" }),
    );
    // 鈥nd that session is what opens, so the user sees the reply they asked for.
    expect(h.store.setActiveSession).toHaveBeenCalledWith("new-session");
  });
  ...
  it("leaves the composer alone when the user navigated to another session mid-send", async () => {
    ...
    // no business clearing. Fire-and-forget 鈥?the reply surfaces as unread.
    const { commitInput, send } = sendFrom(null, () => {
