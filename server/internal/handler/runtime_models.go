...
type ModelListStore interface {
	...
	Complete(ctx context.Context, id string, models []ModelEntry, unavailable []UnavailableModelEntry, supported, fallback bool) error
...
func (s *InMemoryModelListStore) Complete(_ context.Context, id string, models []ModelEntry, unavailable []UnavailableModelEntry, supported, fallback bool) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (h *Handler) ReportModelListResult(w http.ResponseWriter, r *http.Request) {
	...
	if body.Status == "completed" {
		...
		if err := h.ModelListStore.Complete(r.Context(), requestID, body.Models, body.UnavailableModels, supported, body.Fallback); err != nil {
