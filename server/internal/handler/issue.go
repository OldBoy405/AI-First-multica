...
func buildSearchQuery(phrase string, terms []string, queryNum int, hasNum bool, includeClosed bool, terminalStatusKeys []string) (string, []any) {
	...
	// CR-2026-006/CR-2026-009: exclude the hidden Team Agent chat and
	// Discussion container issues. Critical here because the search WHERE also
	// matches on comment content, so without this, chat/discussion messages
	// would leak into global issue search results.
	issueWhere += " AND i.origin_type IS DISTINCT FROM 'project_chat' AND i.origin_type IS DISTINCT FROM 'project_discussion'"
