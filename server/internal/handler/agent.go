...
type AgentTaskResponse struct {
	...
	Kind             string `json:"kind"` // source discriminator: "comment" | "autopilot" | "chat" | "quick_create" | "direct" — quick-create remains stable after its result issue is linked; also used by the activity row to label tasks that have no linked issue
...
func taskToResponse(t db.AgentTaskQueue, workspaceID string) AgentTaskResponse {
	...
	return AgentTaskResponse{
		...
		// issue link for navigation but retains its quick_create kind. The
		// originator (CR-2026-007 DD-1) additionally distinguishes issue-linked
		// tasks from chat-spawned or autopilot-spawned ones; all three may
		// arrive with issue_id = "" once a task has no linked issue.
