...
func (d *Daemon) handleTask(ctx context.Context, task Task, slot int) {
	...
	if task.ChatSessionID != "" {
		taskLog.Info("picked chat task", "chat_session", task.ChatSessionID, "agent", agentName, "provider", provider, "ask_only", task.AskOnly)
	} else {
...
func (d *Daemon) executeAndDrain(ctx context.Context, backend agent.Backend, prompt string, opts agent.ExecOptions, taskLog *slog.Logger, taskID, codexHome string, msgSeq *atomic.Int32) (agent.Result, int32, error) {
	...
	case result := <-session.Result:
		...
		// AIFIRST: AIFI-21 denial-closed loop guard (CR-2026-059).
		//
		// terminalObserved (upstream) outranks a watchdog that fired anyway:
		// if the backend had already read its authoritative result, this is
		// the real outcome and re-tagging it would report a completed run
		// as a hang — but only for the idle_watchdog path; the deny-closed
		// guard's deterministic output means the run cannot have made
		// progress, so its re-tag stays unconditional.
		if denialLoopFired.Load() && !terminalObserved() {
