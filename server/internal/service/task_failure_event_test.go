...
func TestTaskEventCarriesPayloadAndScopeHints(t *testing.T) {
	...
	// CR-2026-008 scope hint: chat-task events are delivered per-user.
	if e.ChatRecipientID != "creator-9" {
		t.Fatalf("event ChatRecipientID = %q, want creator-9", e.ChatRecipientID)
	}
