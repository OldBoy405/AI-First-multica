...
RETURNING id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, squad_id, runtime_mcp_overlay, escalation_for_task_id, fire_at, originator_user_id, runtime_connected_apps, coalesced_comment_ids, delivered_comment_ids, chat_input_task_id, cr_id, pipeline_node_run_id, project_id, chat_finalize_deferred_at, originator_source, delegated_from_task_id, retry_of_task_id, rerun_of_task_id, rule_version_id, trigger_evidence_kind, trigger_evidence_ref_id, accountable_user_id, session_rollout_missing, retired_session_id, quick_actions_disabled, regenerate_quick_actions_for, branch_name
`
...
const countActiveProjectTasksExcluding = `-- name: CountActiveProjectTasksExcluding :one
...
type CountActiveProjectTasksExcludingParams struct {
	ProjectID pgtype.UUID `json:"project_id"`
	ID        pgtype.UUID `json:"id"`
}
...
func (q *Queries) CountActiveProjectTasksExcluding(ctx context.Context, arg CountActiveProjectTasksExcludingParams) (int64, error) {
	row := q.db.QueryRow(ctx, countActiveProjectTasksExcluding, arg.ProjectID, arg.ID)
	var count int64
	err := row.Scan(&count)
	return count, err
}
...
const getActivePipelineTask = `-- name: GetActivePipelineTask :one
...
func (q *Queries) GetActivePipelineTask(ctx context.Context, pipelineNodeRunID pgtype.UUID) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, getActivePipelineTask, pipelineNodeRunID)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
...
SELECT atq.id, atq.agent_id, atq.issue_id, atq.status, atq.priority, atq.dispatched_at, atq.started_at, atq.completed_at, atq.result, atq.error, atq.created_at, atq.context, atq.runtime_id, atq.session_id, atq.work_dir, atq.trigger_comment_id, atq.chat_session_id, atq.autopilot_run_id, atq.attempt, atq.max_attempts, atq.parent_task_id, atq.failure_reason, atq.trigger_summary, atq.force_fresh_session, atq.is_leader_task, atq.wait_reason, atq.initiator_user_id, atq.handoff_note, atq.prepare_lease_expires_at, atq.squad_id, atq.runtime_mcp_overlay, atq.escalation_for_task_id, atq.fire_at, atq.originator_user_id, atq.runtime_connected_apps, atq.coalesced_comment_ids, atq.delivered_comment_ids, atq.chat_input_task_id, atq.cr_id, atq.pipeline_node_run_id, atq.project_id, atq.chat_finalize_deferred_at, atq.originator_source, atq.delegated_from_task_id, atq.retry_of_task_id, atq.rerun_of_task_id, atq.rule_version_id, atq.trigger_evidence_kind, atq.trigger_evidence_ref_id, atq.accountable_user_id, atq.session_rollout_missing, atq.retired_session_id, atq.quick_actions_disabled, atq.regenerate_quick_actions_for, atq.branch_name FROM agent_task_queue atq
