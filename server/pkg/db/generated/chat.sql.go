...
const backfillChatSessionBaseIfNull = `-- name: BackfillChatSessionBaseIfNull :one
UPDATE chat_session
SET base_model = COALESCE(base_model, $3),
    base_thinking_level = COALESCE(base_thinking_level, $4)
WHERE id = $1 AND workspace_id = $2
RETURNING id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, last_read_at, is_agent_intro, pinned_at, project_id, explicitly_created_at, base_model, base_thinking_level, model_override, thinking_level_override, kind
`

type BackfillChatSessionBaseIfNullParams struct {
...
func (q *Queries) BackfillChatSessionBaseIfNull(ctx context.Context, arg BackfillChatSessionBaseIfNullParams) (ChatSession, error) {
	row := q.db.QueryRow(ctx, backfillChatSessionBaseIfNull,
		arg.ID,
		arg.WorkspaceID,
		arg.BaseModel,
...
RETURNING id, chat_session_id, role, content, task_id, created_at, failure_reason, elapsed_ms, message_kind, channel_media_pending_until, channel_ingested, quick_actions, channel_context_revision, channel_outbound_type, channel_outbound_installation_id, channel_outbound_chat_id, channel_outbound_message_ids, author_type, author_id
`
...
VALUES ($1, $2, $3, $4, (SELECT runtime_id FROM agent WHERE id = $2), $5, $6, COALESCE($7::uuid, gen_random_uuid()), $8, $9)
RETURNING id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, last_read_at, is_agent_intro, pinned_at, project_id, explicitly_created_at, base_model, base_thinking_level, model_override, thinking_level_override, kind
`
...
RETURNING task.id, task.agent_id, task.issue_id, task.status, task.priority, task.dispatched_at, task.started_at, task.completed_at, task.result, task.error, task.created_at, task.context, task.runtime_id, task.session_id, task.work_dir, task.trigger_comment_id, task.chat_session_id, task.autopilot_run_id, task.attempt, task.max_attempts, task.parent_task_id, task.failure_reason, task.trigger_summary, task.force_fresh_session, task.is_leader_task, task.wait_reason, task.initiator_user_id, task.handoff_note, task.prepare_lease_expires_at, task.squad_id, task.runtime_mcp_overlay, task.escalation_for_task_id, task.fire_at, task.originator_user_id, task.runtime_connected_apps, task.coalesced_comment_ids, task.delivered_comment_ids, task.chat_input_task_id, task.chat_finalize_deferred_at, task.originator_source, task.delegated_from_task_id, task.retry_of_task_id, task.rerun_of_task_id, task.rule_version_id, task.trigger_evidence_kind, task.trigger_evidence_ref_id, task.accountable_user_id, task.session_rollout_missing, task.retired_session_id, task.quick_actions_disabled, task.regenerate_quick_actions_for, task.branch_name, task.durable_work_dir, task.channel_context_revision, task.comment_thread_id, task.cancelled_by_type, task.cancelled_by_id, task.cancelled_by_name, task.cr_id, task.pipeline_node_run_id, task.project_id, task.approval_workspace_id
`
...
const getActiveProjectSharedSession = `-- name: GetActiveProjectSharedSession :one

SELECT id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, last_read_at, is_agent_intro, pinned_at, project_id, explicitly_created_at, base_model, base_thinking_level, model_override, thinking_level_override, kind FROM chat_session
WHERE workspace_id = $1 AND project_id = $2
  AND kind = 'project_shared' AND status = 'active'
`

type GetActiveProjectSharedSessionParams struct {
	WorkspaceID pgtype.UUID `json:"workspace_id"`
	ProjectID   pgtype.UUID `json:"project_id"`
}
...
func (q *Queries) GetActiveProjectSharedSession(ctx context.Context, arg GetActiveProjectSharedSessionParams) (ChatSession, error) {
	row := q.db.QueryRow(ctx, getActiveProjectSharedSession, arg.WorkspaceID, arg.ProjectID)
	var i ChatSession
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
SELECT id, chat_session_id, role, content, task_id, created_at, failure_reason, elapsed_ms, message_kind, channel_media_pending_until, channel_ingested, quick_actions, channel_context_revision, channel_outbound_type, channel_outbound_installation_id, channel_outbound_chat_id, channel_outbound_message_ids, author_type, author_id FROM chat_message
WHERE id = $1
`
...
func (q *Queries) GetChatMessageInWorkspace(ctx context.Context, arg GetChatMessageInWorkspaceParams) (ChatMessage, error) {
	row := q.db.QueryRow(ctx, getChatMessageInWorkspace, arg.ID, arg.WorkspaceID)
	var i ChatMessage
	err := row.Scan(
		&i.ID,
		&i.ChatSessionID,
...
const getProjectChatSessionForCreator = `-- name: GetProjectChatSessionForCreator :one
SELECT id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, last_read_at, is_agent_intro, pinned_at, project_id, explicitly_created_at, base_model, base_thinking_level, model_override, thinking_level_override, kind FROM chat_session
...
type GetProjectChatSessionForCreatorParams struct {
	ProjectID   pgtype.UUID `json:"project_id"`
	CreatorID   pgtype.UUID `json:"creator_id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
}
...
func (q *Queries) GetProjectChatSessionForCreator(ctx context.Context, arg GetProjectChatSessionForCreatorParams) (ChatSession, error) {
	row := q.db.QueryRow(ctx, getProjectChatSessionForCreator, arg.ProjectID, arg.CreatorID, arg.WorkspaceID)
	var i ChatSession
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const insertProjectSharedSession = `-- name: InsertProjectSharedSession :one
INSERT INTO chat_session (workspace_id, agent_id, creator_id, title, project_id, kind, base_model, base_thinking_level)
VALUES ($1, $4, $2, '', $3, 'project_shared', $5, $6)
RETURNING id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, last_read_at, is_agent_intro, pinned_at, project_id, explicitly_created_at, base_model, base_thinking_level, model_override, thinking_level_override, kind
`

type InsertProjectSharedSessionParams struct {
	WorkspaceID       pgtype.UUID `json:"workspace_id"`
	CreatorID         pgtype.UUID `json:"creator_id"`
	ProjectID         pgtype.UUID `json:"project_id"`
	AgentID           pgtype.UUID `json:"agent_id"`
...
func (q *Queries) InsertProjectSharedSession(ctx context.Context, arg InsertProjectSharedSessionParams) (ChatSession, error) {
	row := q.db.QueryRow(ctx, insertProjectSharedSession,
		arg.WorkspaceID,
		arg.CreatorID,
		arg.ProjectID,
		arg.AgentID,
...
SELECT cs.id, cs.workspace_id, cs.agent_id, cs.creator_id, cs.title, cs.session_id, cs.work_dir, cs.status, cs.created_at, cs.updated_at, cs.unread_since, cs.runtime_id, cs.last_read_at, cs.is_agent_intro, cs.pinned_at, cs.project_id, cs.explicitly_created_at, cs.base_model, cs.base_thinking_level, cs.model_override, cs.thinking_level_override, cs.kind,
...
SELECT message.id, message.chat_session_id, message.role, message.content, message.task_id, message.created_at, message.failure_reason, message.elapsed_ms, message.message_kind, message.channel_media_pending_until, message.channel_ingested, message.quick_actions, message.channel_context_revision, message.channel_outbound_type, message.channel_outbound_installation_id, message.channel_outbound_chat_id, message.channel_outbound_message_ids, message.author_type, message.author_id
FROM chat_message AS message
...
const lockChatSessionInWorkspace = `-- name: LockChatSessionInWorkspace :one
SELECT id, workspace_id, agent_id, creator_id, title, session_id, work_dir, status, created_at, updated_at, unread_since, runtime_id, last_read_at, is_agent_intro, pinned_at, project_id, explicitly_created_at, base_model, base_thinking_level, model_override, thinking_level_override, kind FROM chat_session
WHERE id = $1 AND workspace_id = $2
FOR UPDATE
`

type LockChatSessionInWorkspaceParams struct {
	ID          pgtype.UUID `json:"id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
}
...
func (q *Queries) LockChatSessionInWorkspace(ctx context.Context, arg LockChatSessionInWorkspaceParams) (ChatSession, error) {
	row := q.db.QueryRow(ctx, lockChatSessionInWorkspace, arg.ID, arg.WorkspaceID)
	var i ChatSession
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const patchChatSessionConfig = `-- name: PatchChatSessionConfig :one
UPDATE chat_session
...
type PatchChatSessionConfigParams struct {
	ID                    pgtype.UUID `json:"id"`
	WorkspaceID           pgtype.UUID `json:"workspace_id"`
	ModelOverride         pgtype.Text `json:"model_override"`
	ThinkingLevelOverride pgtype.Text `json:"thinking_level_override"`
...
func (q *Queries) PatchChatSessionConfig(ctx context.Context, arg PatchChatSessionConfigParams) (ChatSession, error) {
	row := q.db.QueryRow(ctx, patchChatSessionConfig,
		arg.ID,
		arg.WorkspaceID,
		arg.ModelOverride,
...
const setChatSessionAgentID = `-- name: SetChatSessionAgentID :execrows
UPDATE chat_session
SET agent_id = $2
WHERE id = $1
`

type SetChatSessionAgentIDParams struct {
	ID      pgtype.UUID `json:"id"`
	AgentID pgtype.UUID `json:"agent_id"`
}
...
func (q *Queries) SetChatSessionAgentID(ctx context.Context, arg SetChatSessionAgentIDParams) (int64, error) {
	result, err := q.db.Exec(ctx, setChatSessionAgentID, arg.ID, arg.AgentID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected(), nil
