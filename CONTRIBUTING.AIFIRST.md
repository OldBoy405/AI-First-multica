...
本仓库是 AI-First 平台的 fork 基座，会长期持续同步上游。前四条规则的目的是把"同步"变成一件双周例行小事，而不是攒到季度才做一次痛苦的大合并；第五、六条是自研代码本身的质量红线，来自架构评审在 openwiki（一个我们只借鉴模式、不引入依赖的参考项目）里看到的反面案例——避免我们自己动手写 Go 代码时复刻同样的问题。
...
## 规则六：错误码注册 + API 参数纪律 + 护栏模块粒度

- **错误码统一注册**：自研包对外抛出的错误码，对标 `crctl.mjs` 的 `fail(CODE, message)` 模式——每个错误码在一处集中声明（如 `server/internal/governance/internal/errcode/`），禁止在各处随手拼字符串当错误码。判断标准：同一类错误（比如"证据漂移"）在不同文件里出现了拼写不同的 code，就是违反了这条。
- **API 参数不得静默忽略**：函数收了一个参数就必须真正生效，或者显式拒绝调用——不能像 openwiki 的 `runOpenWikiAgent(command, cwd, options)` 那样，`options.outputMode` 未设时静默丢弃调用方传入的 `cwd` 改用别的默认值，调用方毫无察觉地传了个不生效的参数。加新参数时问自己一句："如果调用方传了这个值但代码路径没读它，会不会报错？"——不会就是隐患。
- **确定性护栏做成小文件 + 单一职责 + 可单测**：任何"约束 Agent 行为边界"的护栏代码（写白名单、路径校验、格式校验一类），对标 openwiki 的 `docs-only-backend.ts`（2.7 KB）/`okf-middleware.ts`（4.6 KB）的粒度——独立文件、职责单一、能脱离整个 Agent 编排单独跑单测，不要把护栏逻辑塞进大的编排文件里跟业务逻辑混在一起。

## 违反规则怎么办

这不走 CR 治理那套门禁（那是给平台产品需求变更用的，这份约定管的是"我们怎么改这个 fork"）——code review 人工把关：PR 把自研逻辑塞进了上游文件、改了上游文件却没留 `// AIFIRST:` 标记、新文件超 800 行没有拆分理由、绕开归口目录私自重写一份已有 util、随手拼错误码字符串、加了参数却悄悄不生效、或者把护栏逻辑跟业务逻辑混在一个大文件里不可单测，都直接打回重做。
